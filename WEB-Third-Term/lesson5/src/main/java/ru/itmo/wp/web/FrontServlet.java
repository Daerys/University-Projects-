...
public class FrontServlet extends HttpServlet {
    ...
    private Method getMethod(Class<?> pageClass, Route route) {
        Method method = null;
    ...
    private static boolean checkParameters(Class<?>[] a) {
        for (Class<?> c : a) {
            if (!(c.isAssignableFrom(Map.class) || c.isAssignableFrom(HttpServletRequest.class))) {
                return false;
            }
        }
        return true;
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Method method = getMethod(pageClass, route);
        ...
        setTemplate(pageClass, response, view, method, LANGUAGE.equals("en"));
    }

    private void setTemplate(Class<?> pageClass, HttpServletResponse response, Map<String, Object> view,
                                Method method, boolean isDefaultLanguage)
            throws ServletException, IOException {
        ...
        try {
            Template template = newTemplate(pageClass.getSimpleName()
                    + (isDefaultLanguage ? "" : "_" + LANGUAGE) + ".ftlh", isDefaultLanguage);
            template.process(view, response.getWriter());
            return;
        } catch (TemplateException | NullPointerException e) {
            if (sourceConfiguration == null && isDefaultLanguage) {
    ...
    private Template newTemplate(String templateName, boolean isDefaultLanguage) throws ServletException {
        ...
        if (template == null && isDefaultLanguage) {
